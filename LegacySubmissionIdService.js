...
const componentStateGen = new IDGenerator(informixGen, config.ID_SEQ_COMPONENT_STATE)
 ...
 */
async function getChallengeProperties (challengeId, userId, resourceRoleId, phaseId) {
  const result = await informix.query(QUERY_GET_CHALLENGE_PROPERTIES, { challengeId, userId, resourceRoleId, phaseId })
  logger.debug(`Challenge properties for: ${challengeId} are: ${JSON.stringify(result)}`)
  if (!_.isArray(result) || _.isEmpty(result)) {
    throw new Error(`null or empty result get challenge properties for: challengeId ${challengeId}, userId ${userId}, resourceRoleId ${resourceRoleId}, phaseId ${phaseId}`)
  }
  return result[0]
}
...
async function getMMChallengeProperties (ctx, challengeId, userId) {
  ...
  if (!_.isArray(result) || _.isEmpty(result)) {
    throw new Error(`null or empty result get mm challenge properties for : challenge id ${challengeId}, user id ${userId}`)
  }
  return result[0]
}
...
async function addMMSubmission (ctx, challengeId, userId, submissionTime) {
  let [roundId, componentId, componentStateId, numSubmissions, points] = await getMMChallengeProperties(ctx, challengeId, userId)
  ...
  const rrResult = await informix.query(ctx, QUERY_GET_MM_ROUND_REGISTRATION, { roundId, userId })
  if (_.isEmpty(rrResult)) {
    ...
    await informix.query(ctx, QUERY_INSERT_MM_ROUND_REGISTRATION, rrParams)
  } else {
  ...
  if (_.isNumber(componentStateId) && !_.isNaN(componentStateId)) {
    ...
    numSubmissions++
    logger.debug(`increment long_component_state#submission_number by 1, componentStateId: ${componentStateId}, numSubmissions: ${numSubmissions}`)
    await informix.query(ctx, QUERY_UPDATE_LONG_COMPONENT_STATE_NUN_SUBMISSIONS, { componentStateId, numSubmissions })
  } else {
    numSubmissions = 1
    ...
    await informix.query(ctx, QUERY_INSERT_LONG_COMPONENT_STATE, lcsParams)
  }
...
async function addSubmission (newSubmissionId, challengeId, userId, phaseId, url, submissionType, submissionTime, isMM) {
  const [resourceId, value, phaseTypeId, challengeTypeId] = await getChallengeProperties(challengeId,
  ...
  try {
    ...
    if (uploadType === constant.UPLOAD_TYPE['Final Fix']) {
      ...
      if (!isAllowMultipleSubmission) {
        ...
        await informix.query(ctx, QUERY_DELETE_UPLOAD, delParams)
        await informix.query(ctx, QUERY_DELETE_SUBMISSION, delParams)
      }
...
async function updateProvisionalScore (challengeId, userId, phaseId, submissionId, submissionType, reviewScore) {
  ...
  try {
    ...
    const result = await informix.query(ctx, QUERY_GET_SUBMISSION_NUMBER,
      { challengeId, userId, phaseId, submissionId, resourceRoleId: constant.SUBMISSION_TYPE[submissionType].roleId })
    ...
    await informix.query(ctx, QUERY_UPDATE_SUBMISSION_INITIAL_REVIEW_SCORE, { submissionId, reviewScore })

    // Update the submission_points in informixoltp:long_submission table
    await informix.query(ctx, QUERY_UPDATE_LONG_SUBMISSION_SCORE, { componentStateId, submissionNumber, reviewScore })

    // Update the points in informixoltp:long_component_state table
    await informix.query(ctx, QUERY_UPDATE_LONG_COMPONENT_STATE_POINTS, { componentStateId, reviewScore })
...
async function updateFinalScore (challengeId, userId, submissionId, finalScore) {
  ...
  try {
    ...
    // Query roundId
    const [roundId] = await getMMChallengeProperties(ctx, challengeId, userId)
    if (!roundId) {
    ...
    await informix.query(ctx, QUERY_UPDATE_SUBMISSION_FINAL_REVIEW_SCORE, { submissionId, finalScore })

    // Get initial_score from submission table
    let result = await informix.query(ctx, QUERY_GET_SUBMISSION_INITIAL_SCORE, { submissionId })
    const [initialScore] = result[0]

    // Check whether user result exists in informixoltp:long_comp_result
    result = await informix.query(ctx, QUERY_CHECK_COMP_RESULT_EXISTS, { roundId, userId })
    ...
    if (resultExists) {
      ...
      await informix.query(ctx, QUERY_UPDATE_COMP_RESULT_SCORE, params)
    } else {
      // Add entry in long_comp_result table
      logger.debug(`Insert into long_comp_result with params: ${JSON.stringify(params)}`)
      await informix.query(ctx, QUERY_INSERT_COMP_RESULT, params)
    }

    // Update placed
    await informix.query(ctx, QUERY_UPDATE_COMP_RESULT_PLACE, { roundId })
...
async function updateUpload (challengeId, userId, phaseId, url, submissionType, submissionId) {
  ...
  if (submissionId > 0) {
    ...
    const [resourceId] = await getChallengeProperties(challengeId, userId, constant.SUBMISSION_TYPE[submissionType].roleId, phaseId)
...
function handleAxiosError (err, apiName) {
  if (err.response) { // non-2xx response received
    logger.error(`${apiName} Error: ${Flatted.stringify({
...
async function getSubmission (submissionId) {
  try {
    ...
    logger.debug(`Fetched submission for ${submissionId} - ${JSON.stringify(sub.data)}`)
