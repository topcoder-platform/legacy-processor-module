...
// valid resource enum
Joi.resource = () => Joi.alternatives().try(Joi.string().valid('submission'), Joi.string().valid('review'), Joi.string().valid('reviewSummation'))
...
function createEventSchema (payloadKeys) {
  return Joi.object().keys({
    ...
    payload: Joi.object().keys(payloadKeys).required().unknown(true)
  })
}
...
function validateEvent (event, schema) {
  const validationResult = Joi.validate(event, schema, { abortEarly: false, stripUnknown: true })
