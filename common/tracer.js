...
class Spans {
   ...
   * @param {String|Boolean|Number} value Tag value
   */
  setTag (name, value) {
...
/**
 * Creates a new span or child of parent spans
 * @param {String} name
 * @param {Spans} parentSpans
 */
function buildSpans (name, parentSpans = null) {
  const spans = parentSpans ? startChildSpans(name, parentSpans) : startSpans(name)
  return spans
}

/**
 * Logs an error for the given span
 * @param {Spans} span
 * @param {Error} err
 */
function logSpanError (span, err) {
  span.log({
    event: 'error',
    message: err.message,
    stack: err.stack,
